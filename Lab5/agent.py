...
=================================
agent.py 將 Agent 執行與 Log 整理分開處理：先讓 Agent 依讀者問題自主選擇檢索工具，
再視顯示需求將 Agent 訊息整理成 Log 與回答事件，交給 main.py 顯示。

程式流程：
  1. stream_agent() 建立 Agent、送出讀者問題，並回傳 Agent 逐步產生的更新資料。
  2. _iter_messages() 從逐步產生的更新資料中取出 AIMessage 與 ToolMessage。
  3. _iter_log_events() 將訊息整理成 Log 與最終回答事件。
  4. run_agentic_rag() 串接上述流程，並將事件交給 main.py 顯示。
"""
...
# ── 給 AI Agent 的 Prompt：工具選擇原則、資料不足時的處理、何時使用 skill 等───────────────────────────────────────────────
...
一旦確認本輪是匯出請求，只能使用對話中最近一次已檢索、已推薦的書籍資料，不得重新呼叫
hyde_query、vector_retrieve 或 graph_retrieve；寫檔成功後必須立即告知實際檔名與路徑並結束本輪，
不得繼續推理、重新判斷問題類型或呼叫任何其他工具。
    ...
    for update in agent_updates:
        ...
        for node_result in update.values():
            ...
            if isinstance(node_result, dict) and "messages" in node_result:
                # 逐筆取出 AIMessage 或 ToolMessage，交給後續程式處理。
                for message in node_result["messages"]:
                    yield message


# ── 將 AI Agent 訊息整理成發生事件 Log ──────────────────────────────────────────────

def _iter_log_events(messages) -> Iterator[dict]:
    # 暫存尚未收到工具結果的工具呼叫，key 為 tool_call_id。
    pending_calls = {}
    # 記錄要顯示給使用者的 Log 編號，從 1 開始。
    activity_index = 0
    # 保存最後一則沒有 tool_calls 的 AIMessage，串流結束後即為最終回答。
    final_answer = None

    # 逐筆將 AIMessage 與 ToolMessage 整理成 main.py 需要的事件。
    for message in messages:
        ...
        if isinstance(message, ToolMessage):
            ...
            # 非 Skill 檔案或其他不需顯示的內建工具不會建立事件，因此略過。
            if event is None:
                continue

            # 每產生一個 Log 事件就加上編號，再交給 main.py 顯示。
            activity_index += 1
...
def run_agentic_rag(query: str) -> Iterator[dict]:
    try:
        # 核心 Agent 只負責執行問題並回傳逐步產生的更新資料。
        agent_updates = stream_agent(query)

        # 從逐步產生的更新資料中取出 AIMessage 與 ToolMessage。
        messages = _iter_messages(agent_updates)

        # 將 Agent 訊息整理成 main.py 可以顯示的 Log 與回答事件。
        log_events = _iter_log_events(messages)
