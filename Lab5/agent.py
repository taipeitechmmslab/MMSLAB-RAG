...
def run_agentic_rag(query: str) -> dict:
    ...
    try:
        ...
        # 呼叫前先記下這個 thread 目前已累積的訊息數，執行後用來切出「這一輪新增」的訊息；
        # 第一輪呼叫時這個 thread 還沒有任何 checkpoint，回傳空狀態，prior_count 為 0
        prior_state = agent.get_state(config)
        prior_count = len(prior_state.values.get("messages", [])) if prior_state.values else 0
        # 只需帶入這一輪新的 HumanMessage，LangGraph 會依 thread_id 自動接續先前輪次的歷史訊息；
    ...
    # result["messages"] 是這個 thread 累積至今的完整訊息紀錄（含先前輪次），
    # 切出這一輪新增的部分，避免 retrievals 重複列出先前輪次已經印過的工具呼叫
    ...
    # 依序走訪這一輪新增的訊息，把 AIMessage 的 tool_calls 與對應 ToolMessage 的結果配對還原成 retrievals 過程
