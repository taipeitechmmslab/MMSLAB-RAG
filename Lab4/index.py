...
def split_documents(documents: list[Document]) -> list[Document]:
    ...
    # chunk_size 控制每段文字長度；chunk_overlap 讓相鄰 chunks 保留部分重疊內容，避免切分時遺失上下文
...
def build_vector_store(chunks: list[Document]) -> Milvus:
    """
    使用 NVIDIA NIM Embedding Model 將 chunks 向量化，並透過 Milvus.from_documents
    建立 library_books collection 存入向量資料庫。
    """
    ...
    vector_store = Milvus.from_documents(
        ...
        drop_old=True,
        # 允許 metadata 欄位動態寫入，避免為每個書籍欄位預先定義 schema
